...
export async function POST(request: NextRequest) {
  try {
    ...
    if (provider === 'openai') {
      ...
      const openaiResponse = await fetch(`${request.nextUrl.origin}/api/generate-image-openai`, {
        method: 'POST',
      ...
      const fluxResponse = await fetch(`${request.nextUrl.origin}/api/generate-image-flux`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(body)
      })
      ...
      const geminiResponse = await fetch(`${request.nextUrl.origin}/api/generate-image-gemini`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(body)
      })
